...
export default function ConciliacaoPage() {
  ...
  // =========================================================================
  // MOTOR DE EXTRAÇÃO INTELIGENTE (ESTOQUE + CONTROLO DE DESPESAS)
  // =========================================================================
  const processarInsercaoNoEstoque = async (itens: any[], fornecedor: string, mesRef: string) => {
    ...
    for (const item of itens) {
      try {
        ...
        // 1. ATUALIZAÇÃO DO ESTOQUE / ARMAZÉM
        ...
        // 2. LANÇAMENTO NAS DESPESAS (Mapeado exatamente com a sua página de Despesas)
        let categoriaDespesa = 'Outros'; 
        
        if (tipoItem === 'alimentar' || tipoItem === 'insumo' || tipoItem === 'mercado') categoriaDespesa = 'Ingredientes & Mercadoria';
        else if (tipoItem === 'embalagem' || tipoItem === 'etiqueta') categoriaDespesa = 'Embalagens & Consumíveis';
        else if (tipoItem === 'marketing' || tipoItem === 'publicidade' || tipoItem === 'anuncio') categoriaDespesa = 'Marketing & Publicidade';
        else if (tipoItem === 'plataforma' || tipoItem === 'taxa' || tipoItem === 'comissao' || tipoItem === 'glovo' || tipoItem === 'palmbites') categoriaDespesa = 'Estrutura & Fixos';
        else if (tipoItem === 'combustivel' || tipoItem === 'transporte' || tipoItem === 'estafeta') categoriaDespesa = 'Frota & Combustível';
        else if (tipoItem === 'devolucao' || tipoItem === 'reembolso') categoriaDespesa = 'Devoluções & Reembolsos';

        const descFinal = item.quantidade > 1 ? `${item.nome_extraido} (${item.quantidade} ${item.unidade || 'un'}) - ${fornecedorFormatado}` : `${item.nome_extraido} - ${fornecedorFormatado}`;
        ...
        await supabase.from('despesas').insert([{
          ...
          status: 'Validado' // Faturas que o sistema lê assumimos sempre como validadas!
        }]);
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (!res.ok) {
           ...
           if (dataAPI.dadosLidos) {
             ...
             // Se for fatura geral que a IA não separou linha a linha
    ...
    <div className="p-8 font-sans max-w-7xl mx-auto relative">
      <div className="mb-8 border-b border-zinc-800 pb-4">
        <h1 className="text-3xl font-bold text-orange-500 flex items-center gap-3">
          Conciliador Inteligente <span className="bg-orange-500 text-white text-xs px-2 py-1 rounded-full">v2.3</span>
        </h1>
