...
export default function DespesasPage() {
  ...
  // 🎯 NOVO: Estado para controlar a ordenação da lista
  ...
  async function carregarDespesas() {
    ...
    // 🎯 NOVO: Lógica de Ordenação Múltipla
    if (ordenacao === 'data_desc') query = query.order('data_despesa', { ascending: false });
    else if (ordenacao === 'data_asc') query = query.order('data_despesa', { ascending: true });
    else if (ordenacao === 'alfabetica_asc') query = query.order('descricao', { ascending: true });
    else if (ordenacao === 'alfabetica_desc') query = query.order('descricao', { ascending: false });
    else if (ordenacao === 'valor_desc') query = query.order('valor', { ascending: false });
    else if (ordenacao === 'valor_asc') query = query.order('valor', { ascending: true });
    else if (ordenacao === 'categoria_asc') query = query.order('categoria', { ascending: true });
  ...
  const getCategoriaCor = (categoria: string) => {
    ...
    if (cat.includes('ingredientes') || cat.includes('mercado')) return 'bg-blue-500/20 text-blue-400 border-blue-500/30';
  ...
  return (
    <div className="p-8 font-sans max-w-7xl mx-auto relative min-h-screen">
      ...
      <div className="flex justify-end mb-4">
        <div className="flex items-center gap-3 bg-[#121214] px-4 py-2.5 rounded-xl border border-zinc-800 shadow-md">
          <span className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Ordenar por:</span>
          <select 
            value={ordenacao} 
      ...
      <div className="bg-zinc-900/90 border border-zinc-800/80 rounded-[24px] overflow-hidden shadow-2xl">
        <div className="p-5 border-b border-zinc-800/80 bg-zinc-950/40 flex justify-between items-center">
          <h3 className="text-xs font-extrabold text-zinc-400 uppercase tracking-widest">Listagem Detalhada de Itens</h3>
        </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex flex-col justify-center items-center p-4 animate-in fade-in duration-200">
          <div className="bg-zinc-900 w-full max-w-lg rounded-[32px] flex flex-col overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-zinc-800">
            <div className="p-6 pb-4 flex justify-between items-center border-b border-zinc-800 bg-zinc-950/50">
              <h2 className="text-xl font-black text-white">✏️ Classificar Despesa</h2>
            ...
            <form onSubmit={salvarEdicao} className="p-6 space-y-5">
              <div>
                <label className="block text-[10px] text-zinc-400 font-black uppercase tracking-widest mb-2">Descrição do Item</label>
              ...
              <div>
                ...
                <select value={form.categoria} onChange={e => setForm({...form, categoria: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl px-4 py-3 text-sm text-white outline-none focus:border-blue-500 appearance-none cursor-pointer">
                  {/* 🎯 NOVAS CATEGORIAS A PEDIDO */}
                  <option value="Mercados & Ingredientes">Mercados & Ingredientes</option>
